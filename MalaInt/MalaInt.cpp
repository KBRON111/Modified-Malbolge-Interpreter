...
static inline
#endif
void exec(unsigned short *mem, FILE* normalizationFile, bool simple);
void DumpMemory(FILE* f, unsigned short *mem);
#ifdef __GNUC__
static inline
#endif
...
int main(int argc, char **argv)
{
	...
	FILE *normalizedOpFile = NULL;
	if (argc >= 3)
	{
		if ((normalizedOpFile = fopen(argv[2], "w")) == NULL)
		{
			fputs("failed to open normalization file", stderr);
			return (1);
		}
	}
	...
	{
		FILE *dumpFile = NULL;
		if ((dumpFile = fopen(argv[3], "w")) == NULL)
		{
			fputs("failed to open dump file", stderr);
			return (1);
		}
		DumpMemory(dumpFile, mem);
		fclose(dumpFile);
	}
	
	exec(mem, normalizedOpFile, true);
	free(mem);
	if (normalizedOpFile)
		fclose(normalizedOpFile);
	return (0);
}
...
void exec(unsigned short *mem, FILE* normalizationFile, bool simple)
{
	...
	for (;;)
	{
		...
		if (mem[c] < 33 || mem[c] > 126)
		{
			...
			{
				fprintf(normalizationFile, "Invalid command. Looping infinitely.");
				fclose(normalizationFile);
				normalizationFile = NULL;
			}
			continue; 
		}
		...
		if (normalizationFile)
		{
			if (simple)
			{
				fprintf(normalizationFile, "%c\n", currentOp);
			}
			else //Not implemented via command line, this is a more detailed look at the memory in each step
			{
				fprintf(normalizationFile, "op:%c a:%d c:%d [c]:%d d:%d [d]:%d\n", currentOp, a, c, mem[c], d, mem[d]);
			}
...
void DumpMemory(FILE* f, unsigned short *mem)
{
	...
	fprintf(f, "Memory Dump at %s\n", buffer);
	for (int addr = 0; addr < 59049; addr++)
	{
		fprintf(f, " %d ", mem[addr]);
		if (addr != 0 && addr % 8 == 0)
			fprintf(f, "\n");
	}
}
